...
const TogglePaiementLevelTwo = (props) => {
  const { user } = props
  const [isChecked, setIsChecked] = useState(user.isLevelTwoPayed)
  ...
  const checkHandler = async () => {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ isLevelTwoPayed: !isChecked }),
    })
...
export default TogglePaiementLevelTwo
